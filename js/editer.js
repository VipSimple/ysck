...
var articleType = 0;   //文章类型   0: 个人  1: 公共
var publicForType = 0;  //公共 默认为否
var selfForType = 0;  //个人 默认为否
...
var tmplForImg = '<div class="edit-article-item newItem current-article-item">'+
		...
		'<div class="edit-add_bottom">'+
			...
			'<div class="edit-add-check-bottom">'+
				'<div class="edit-add-option_text">文本</div>'+
				'<div class="edit-add-option_img"><span>图片</span></div>'+
			'</div>'+
		'</div>'+
		'<div class="pos-arrow arrow_top">^</div>'+
		'<div class="pos-arrow arrow_bottom">^</div>'+
	'</div>';
var tmplForText = '<div class="edit-article-item newItem current-article-item">'+
		'<div class="edit-article-img">'+
...
function removeEvent(){
	...
	$('.arrow_bottom, .arrow_top').unbind('click');
...
function bindEvent(){
	...
	$('#submitTitleBtn').bind('click',function(){
		...
		// }else{
			...
			var curScene = $('#titleBox');
			var nextScene = $('.edit-container');
			curScene.removeClass('scene-active').addClass('scene-next scene-out');
			nextScene.removeClass('scene-prev').addClass('scene-active scene-in');
		// }
	...
	$('.edit-add-option_img span').bind('click',function(e){
		$('.edit-add-option_img span').unbind('click');
		console.log($(this));
	...
	$('input[type=file]').bind('change',function(e){
		...
		// console.log( $(this).parents('.edit-article-item').index() );
	...
	$('.edit-article-delete').bind('click',function(){
		...
		if($('.edit-article-item').length == 1) {

			var $tooltips = $('.js_tooltips');
            if ($tooltips.css('display') != 'none') return;

            $tooltips.html('至少保留一张图片')
	...
	$(".weui-dialog__btn").bind('click',function(){
...
function addItem(src){
	...
	if(isBeforeInsert){
		...
		if(!isTextItem){
			...
			// 进场动画
			$('.edit-article-item.newItem').offset({left: innerWidth});
			
			if(timer != 'undefined'){ timer = undefined }
				timer = setTimeout(function(){

				$('.edit-article-item.newItem').removeClass('newItem').offset({left:0});
			...
			curItem.before( $(tmplForText.replace('#text#', textInputVal)) );
			console.log('textItem');
			// 进场动画
			$('.edit-article-item.newItem').eq(0).offset({left: innerWidth});
		...
		if(!isTextItem){
			...
			curItem.after( $(tmplForText.replace('#text#', textInputVal)) );
			console.log('bottom_textItem');
			// 进场动画
			$('.edit-article-item.newItem').eq(0).offset({left: innerWidth});
...
function changeImg(src){
	...
	if(that.parents('.js_limit').length > 0){
		that.parents('.js_limit').removeClass('js_limit');
	}
}
...
// 	// 改变标题名 
// 	if(isEditArticle){
// 		$('.layerTitle').html('编辑文章');

// 		$('.layerWrap, .shade').show();

// 	}else{
// 		$('.layerTitle').html('编辑标题');
...
function confirmLayerWrap(){
	...
	var curScene = $('.edit-container');
	var nextScene = $('#textBox');
	curScene.removeClass('scene-active').addClass('scene-prev scene-out');
	nextScene.removeClass('scene-next').addClass('scene-active scene-in');
...
function submitText(){
	...
	// if(isEditArticle){
		if(isTextItem){
			...
			if($('.js_limit').length > 0){
				$('.js_limit').removeClass('js_limit');
			}
...
$('#submitItemBtn').bind('click',function(){
	...
	if(userInfoIsOver){
		...
		// 判断是否编辑过
		if($('.edit-article-item').eq(0).hasClass('js_limit')){

			var $tooltips = $('.js_tooltips');

            if ($tooltips.css('display') != 'none') return;

            $tooltips.html('请选上传一张图，或者编辑一段文字')
            $tooltips.css('display', 'block');
...
$('#userSubmitBtn').bind('click',function(){
	...
	if($('input#sele:checked').length > 0){
		// 个人
		articleType = 0;
	}else{
		articleType = 1;
	...
	switch(articleType){
		case 0:
			// 用户名为空
			if($('.userName').val() == ''){
				$('.name-tips').css({'bottom':'-6px','opacity':1,'visibility': 'visible;'})
				$('.userName').focus();
				return ;
			}
			// 手机号为空
			if($('.userTel').val() == ''){
				$('.tel-tips').css({'bottom':'-6px','opacity':1,'visibility': 'visible;'})
				$('.userTel').focus();
				return;
			}
			// 手机号正确
			if( isMobile($('.userTel').val()) ){

				// 提交信息 --

			}else{
			// 手机号不正确
				$('.tel-tips').html('请输入正确的手机号！').css({'bottom':'-6px','opacity':1,'visibility': 'visible;'})
				$('.userTel').focus();
				return;
			}
		...
		case 1:
			...
			if($('.userTel').val() == ''){
				...
				// 手机号正确
				if( isMobile($('.userTel').val()) ){
					$('.tel-tips').css({'bottom': '-14px','opacity':0,'visibility':'hidden'})
